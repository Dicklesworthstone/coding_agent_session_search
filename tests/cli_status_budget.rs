...
fn triage_emits_complete_budget_block_with_optional_guidance() -> Result<(), String> {
    ...
    require(
        ...
        "complete triage did not mark derived readiness projections as inspected",
    )
}

#[test]
fn triage_introspection_requires_the_typed_budget_block() -> Result<(), String> {
    let output = Command::new(cass_bin())
        ...
        .map_err(|error| error.to_string())?;
    let json = parse_stdout_json(&String::from_utf8_lossy(&output.stdout));
    let triage_schema = &json["response_schemas"]["triage"];
    let budget_schema = &triage_schema["properties"]["budget"];
    let required = triage_schema["required"]
        .as_array()
        .ok_or_else(|| "triage schema required list is missing".to_string())?;

    require(
        budget_schema["type"] == "object"
            && budget_schema["properties"]["elapsed_ms"]["type"] == "integer"
            && budget_schema["properties"]["budget_ms"]["type"] == "integer"
            && budget_schema["properties"]["timed_out"]["type"] == "boolean"
            && budget_schema["properties"]["skipped_sections"]["type"] == "array"
            && budget_schema["properties"]["recommended_next_probe"]["type"].is_array(),
        "triage introspection omitted the typed budget contract",
    )?;
    require(
        required.iter().any(|field| field == "budget"),
        "triage introspection does not require the runtime budget block",
    )?;
    require(
        triage_schema["properties"]["search_completeness"]["properties"]["inspected"]["type"]
            == "boolean"
            && triage_schema["properties"]["search_completeness"]["properties"]["complete"]["type"]
                .is_array()
            && triage_schema["properties"]["root_cause"]["properties"]["inspected"]["type"]
                == "boolean"
            && triage_schema["properties"]["readiness"]["properties"]["pending"]["properties"]["inspected"]
                ["type"]
                == "boolean",
        "triage introspection cannot describe an explicitly uninspected partial readiness result",
    )
}

#[test]
fn triage_hard_bounds_an_actually_slow_readiness_probe() -> Result<(), String> {
    let tmp = TempDir::new().map_err(|error| error.to_string())?;
    seed_minimal_archive(tmp.path())?;
    let data_dir = tmp.path().to_string_lossy().to_string();
    let started = Instant::now();
    let output = triage_cmd(&data_dir)
        .args(["--stale-threshold", "7"])
        .env("CASS_TRIAGE_BUDGET_MS", "150")
    ...
    require(
        ...
        &format!(
            "triage waited for the full 2500ms readiness fixture instead of enforcing its 150ms budget (wall={wall_clock:?})"
        ),
    )?;
    for section in [
        "readiness_state",
    ...
    require(
        budget["elapsed_ms"]
            .as_u64()
            .is_some_and(|value| value >= 150 && value < 900)
            && budget["timed_out"] == true,
...
#[test]
fn triage_hard_bounds_core_filesystem_probes_without_fabricating_facts() -> Result<(), String> {
    let tmp = TempDir::new().map_err(|error| error.to_string())?;
    seed_minimal_archive(tmp.path())?;
    let data_dir = tmp.path().to_string_lossy().to_string();
    let started = Instant::now();
    ...
    require(
        wall_clock < Duration::from_millis(900),
        &format!(
            "triage waited for the full 2500ms core-filesystem fixture instead of enforcing its 150ms budget (wall={wall_clock:?})"
        ),
    )?;
    require(
        budget["timed_out"] == true
            && budget["elapsed_ms"].as_u64().is_some_and(|value| value >= 150)
            && budget["skipped_sections"]
                .as_array()
...
#[test]
fn triage_hard_bounds_an_actually_slow_database_count_scan() -> Result<(), String> {
    let tmp = TempDir::new().map_err(|error| error.to_string())?;
    seed_minimal_archive(tmp.path())?;
    let data_dir = tmp.path().to_string_lossy().to_string();
    let started = Instant::now();
    let output = triage_cmd(&data_dir)
        .env("CASS_TRIAGE_BUDGET_MS", "500")
    ...
    require(
        ...
        &format!(
            "triage waited for the full 3000ms count fixture instead of enforcing its 500ms budget (wall={wall_clock:?})"
        ),
    )?;
    require(
    ...
    require(
        budget["timed_out"] == true
            && budget["elapsed_ms"].as_u64().is_some_and(|value| value >= 500),
        "triage count hard deadline did not report a timeout",
    )?;
    require(
    ...
    require(
        budget["recommended_next_probe"]
            .as_str()
            .is_some_and(|probe| probe.contains("cass triage --json")),
        "triage count timeout did not expose a recovery probe",
    )
}
