...
use coding_agent_search::raw_mirror::{RawMirrorCaptureInput, capture_source_file, storage_summary};
...
fn fixture(path: &Path) -> Connection {
    ...
    for (number, name) in [(1, "001-conversations.sql"), (2, "002-messages.sql"),
        (3, "003-add-message-sequence.sql")] {
        conn.execute_compat("INSERT INTO migrations (migration_number,migration_name) VALUES (?,?)",
            params![number, name]).unwrap();
    }
...
fn insert_session(conn: &Connection, id: &str, text: &str) {
    conn.execute_compat("INSERT INTO conversations (conversation_id,slug,cwd,created_at,updated_at)
        VALUES (?,?, '/work/original', '2026-08-01 10:00:00', '2026-08-01 10:00:00')",
        params![id, format!("title-{id}")]).unwrap();
...
fn insert_child(conn: &Connection, id: &str, text: &str) {
    ...
    conn.execute_compat("UPDATE conversations SET parent_conversation_id='alpha' WHERE conversation_id=?",
        params![id]).unwrap();
}
...
fn bundle(path: &Path) -> SourceBundle {
    ["", "-wal", "-shm"].into_iter().map(|suffix| {
        ...
        path.exists().then(|| (fs::read(&path).unwrap(), fs::metadata(path).unwrap().modified().unwrap()))
    }).collect()
}

fn command(home: &Path, data: &Path, db: &Path) -> Command {
    let mut cmd = Command::new(assert_cmd::cargo::cargo_bin!("cass"));
    cmd.env_clear().env("HOME",home).env("USERPROFILE",home).env("PATH","")
        .env("XDG_CONFIG_HOME", home.join(".config"))
        .env("XDG_DATA_HOME", home.join(".local/share"))
        .env("CASS_SHELLEY_DB", db).env("CASS_DATA_DIR",data).env("CASS_IGNORE_SOURCES_CONFIG","1")
        .env("CASS_AUTO_REFRESH","0").env("CODING_AGENT_SEARCH_NO_UPDATE_PROMPT","1")
        .env("RUST_MIN_STACK","134217728").current_dir(home);
    if let Ok(system_root)=dotenvy::var("SystemRoot") {cmd.env("SystemRoot",system_root);}
    cmd
}
...
fn search(home: &Path, data: &Path, db: &Path, needle: &str, workspace: Option<&str>) -> Value {
    ...
    cmd.args(["search",needle,"--mode","lexical","--json","--no-maintenance",
        "--timeout","3000","--limit","100"]);
    if let Some(workspace)=workspace {cmd.args(["--workspace",workspace]);}
    let output=cmd.timeout(Duration::from_secs(10)).assert().success().get_output().stdout.clone();
    let value:Value=serde_json::from_slice(&output).unwrap();
    assert!(value["hits"].is_array(),"missing hits: {value}");
    assert_ne!(value.pointer("/budget/timed_out").and_then(Value::as_bool),Some(true),
...
fn canonical(data: &Path) -> Vec<CanonicalSession> {
    ...
    storage.raw().query("SELECT c.id,c.external_id,c.title,w.path FROM conversations c
        JOIN agents a ON a.id=c.agent_id JOIN workspaces w ON w.id=c.workspace_id
        WHERE a.slug='shelley' ORDER BY c.external_id").unwrap().into_iter().map(|row| {
            let id=row.get_typed::<i64>(0).unwrap();
            (id,row.get_typed::<String>(1).unwrap(),row.get_typed::<String>(2).unwrap(),
...
fn analytics_without_workspace(data: &Path) -> Vec<String> {
    let storage=SqliteStorage::open_readonly(&data.join("agent_search.db")).unwrap();
    ["message_metrics", "token_usage"].into_iter().map(|table| {
        let columns:Vec<String>=storage.raw().query(&format!("PRAGMA table_info({table})"))
            .unwrap().iter().map(|row|row.get_typed::<String>(1).unwrap())
            .filter(|column|column!="workspace_id").collect();
        let rows=storage.raw().query(&format!("SELECT {} FROM {table} ORDER BY message_id",columns.join(","))).unwrap();
        assert!(!rows.is_empty(),"{table} must contain real ingestion analytics");
        format!("{rows:?}")
    }).collect()
}
...
fn assert_analytics_workspace_matches_canonical(data: &Path) {
    ...
    for table in ["message_metrics", "token_usage"] {
        let rows=storage.raw().query(&format!("SELECT COUNT(*) FROM {table} t
            JOIN messages m ON m.id=t.message_id JOIN conversations c ON c.id=m.conversation_id
            WHERE t.workspace_id != c.workspace_id")).unwrap();
        assert_eq!(rows[0].get_typed::<i64>(0).unwrap(),0,"{table} kept obsolete workspace");
    }
}
...
fn shelley_local_read_conserves_source_and_sensitive_capture_is_denied() {
    ...
    let local=ScanContext::with_roots(home.path().join("data"),vec![ScanRoot::local(db.clone())],None);
    let conversations=connector.scan(&local).unwrap();
    assert_eq!(conversations.len(),2);
    assert!(conversations.iter().all(|c|c.agent_slug=="shelley" && c.messages.len()==1 && c.messages[0].idx==7));
    assert_eq!(bundle(&db),before,"local reader mutated provider files");
    let remote=ScanContext::with_roots(home.path().join("data"),vec![ScanRoot::remote(
        db.clone(),franken_agent_detection::types::Origin::remote("fixture-host"),None)],None);
    ...
    for origin in ["local","remote"] {
        for suffix in ["","-wal","-shm"] {
            ...
            let error=capture_source_file(RawMirrorCaptureInput {data_dir:&data,provider:"shelley",
                source_id:"fixture",origin_kind:origin,origin_host:None,source_path:&source,db_links:&[]}).unwrap_err();
            assert!(error.to_string().contains("disabled_sensitive_container"),"{error}");
        }
    }
...
fn shelley_batch_refreshes_metadata_without_replacing_messages_or_mirroring_database() {
    ...
    for round in 0..3 {
        if round==1 {
            writer.execute_compat("UPDATE conversations SET slug='renamed-alpha',cwd='/work/renamed',
                archived=1,tags='[\"reviewed\"]' WHERE conversation_id='alpha'",params![]).unwrap();
        }
        let before=bundle(&db);
        assert_cmd::Command::from_std(command(home.path(),&data,&db))
            .args(["index","--full","--json"]).timeout(Duration::from_secs(120)).assert().success();
        let rows=canonical(&data);
        assert_eq!(rows.len(),2,"shared database must retain distinct session identities");
        if round==0 {original=rows.clone();original_analytics=analytics_without_workspace(&data);} else {
            assert_eq!(analytics_without_workspace(&data),original_analytics,
            ...
            for (old,new) in original.iter().zip(&rows) {
                assert_eq!((&old.0,&old.1,&old.4),(&new.0,&new.1,&new.4),"metadata refresh replaced canonical identity/messages");
            }
            let alpha=rows.iter().find(|r|r.1.ends_with(":alpha")).unwrap();
            assert_eq!(alpha.2,"renamed-alpha"); assert_eq!(alpha.3,"/work/renamed");
            let archive=SqliteStorage::open_readonly(&data.join("agent_search.db")).unwrap();
            let saved=archive.list_conversations(20,0).unwrap().into_iter()
                .find(|c|c.external_id.as_deref().is_some_and(|id|id.ends_with(":alpha"))).unwrap();
            assert_eq!(saved.metadata_json["shelley"]["archived"],true);
            assert_eq!(saved.metadata_json["shelley"]["tags"],json!(["reviewed"]));
            drop(archive);
            assert_eq!(search(home.path(),&data,&db,"shelleyalphaneedle",Some("/work/renamed"))["hits"].as_array().unwrap().len(),1);
            assert!(search(home.path(),&data,&db,"shelleyalphaneedle",Some("/work/original"))["hits"].as_array().unwrap().is_empty());
        }
        ...
        assert_eq!(storage_summary(&data).manifest_count,0,"sensitive container leaked into raw mirror");
...
fn shelley_live_watch_reads_wal_only_new_session_and_metadata_change() {
    ...
    impl Drop for WatchChild {fn drop(&mut self){let _=self.0.kill();let _=self.0.wait();}}
    let home=tempfile::tempdir().unwrap();let db=home.path().join("custom-store.sqlite3");
    let data=home.path().join("data");let writer=fixture(&db);
    insert_child(&writer,"initial-child","shelleyinitialchildneedle");
    let stdout=home.path().join("watch.stdout");let stderr=home.path().join("watch.stderr");
    let logs=||format!("stdout:\n{}\nstderr:\n{}",fs::read_to_string(&stdout).unwrap_or_default(),fs::read_to_string(&stderr).unwrap_or_default());
    let mut watch=WatchChild(command(home.path(),&data,&db).env("RUST_LOG","info")
        .env("CASS_SKIP_SUBAGENTS","1").arg("--verbose")
    ...
    while !logs().contains("watch mode: minimum interval between scan cycles") {
        assert!(watch.0.try_wait().unwrap().is_none(),"watch exited: {}",logs());
        assert!(Instant::now()<startup,"watch startup timeout: {}",logs());
        std::thread::sleep(Duration::from_millis(100));
    }
    let original=canonical(&data);assert_eq!(original.len(),2);
    ...
    assert_eq!(fs::read(&db).unwrap(),main_before,"fixture commit must remain WAL-only");
    let committed=bundle(&db);let deadline=Instant::now()+Duration::from_secs(60);
    loop {
        let rows=canonical(&data);
        if rows.len()==3 && rows.iter().any(|r|r.1.ends_with(":alpha") && r.2=="live-renamed" && r.3=="/work/live") {
            for old in &original {
            ...
            let gamma=search(home.path(),&data,&db,"shelleygammaneedle",None);
            let alpha=search(home.path(),&data,&db,"shelleyalphaneedle",Some("/work/live"));
            if gamma["hits"].as_array().unwrap().len()==1
                && alpha["hits"].as_array().unwrap().len()==1 { break; }
        }
        assert!(watch.0.try_wait().unwrap().is_none(),"watch exited: {}",logs());
        assert!(Instant::now()<deadline,"WAL-only session/metadata not indexed: {}",logs());
        std::thread::sleep(Duration::from_millis(200));
    }
    for needle in ["shelleyalphaneedle","shelleybetaneedle","shelleygammaneedle"] {
        assert_eq!(search(home.path(),&data,&db,needle,None)["hits"].as_array().unwrap().len(),1);
    }
    assert_eq!(search(home.path(),&data,&db,"shelleyalphaneedle",Some("/work/live"))["hits"].as_array().unwrap().len(),1);
    assert!(search(home.path(),&data,&db,"shelleyalphaneedle",Some("/work/original"))["hits"].as_array().unwrap().is_empty());
    for needle in ["shelleyinitialchildneedle","shelleylaterchildneedle"] {
        assert!(search(home.path(),&data,&db,needle,None)["hits"].as_array().unwrap().is_empty(),
            "subagent exclusion must apply during initial scan and WAL-triggered scan");
    }
    assert_eq!(bundle(&db),committed,"watch reader mutated provider DB/WAL/SHM");
...
fn shelley_subagent_toggle_applies_to_batch_and_streaming() {
    for streaming in ["0","1"] {
        for skip in ["0","1"] {
            ...
            let before=bundle(&db);
            assert_cmd::Command::from_std(command(home.path(),&data,&db))
                .env("CASS_STREAMING_INDEX",streaming).env("CASS_SKIP_SUBAGENTS",skip)
                .args(["index","--full","--json"]).timeout(Duration::from_secs(120))
                .assert().success();
            let rows=canonical(&data);
            assert_eq!(rows.len(),if skip=="1" {2} else {3},"streaming={streaming} skip={skip}");
            assert_eq!(search(home.path(),&data,&db,"shelleychildtoggleproof",None)["hits"]
                .as_array().unwrap().len(),usize::from(skip=="0"));
            assert_eq!(search(home.path(),&data,&db,"shelleyalphaneedle",None)["hits"]
                .as_array().unwrap().len(),1,"parent must remain searchable");
            assert_eq!(bundle(&db),before);
        }
    }
}
...
fn shelley_configured_local_file_scans_only_the_requested_database() {
    ...
    let home=tempfile::tempdir().unwrap();
    let provider=home.path().join("provider");fs::create_dir_all(&provider).unwrap();
    let db=provider.join("chosen.sqlite3");let _writer=fixture(&db);
    let neighbor=provider.join("neighbor.sqlite3");let _neighbor_writer=fixture(&neighbor);
    let before=bundle(&db);let neighbor_before=bundle(&neighbor);
    let data=home.path().join("data");
    ...
    let config=SourcesConfig {
        ...
        disabled_agents:coding_agent_search::connectors::get_connector_factories()
            .into_iter().map(|(name,_)|name.to_string()).filter(|name|name!="shelley").collect(),
    };
    ...
    cmd.env_remove("CASS_SHELLEY_DB").env_remove("CASS_IGNORE_SOURCES_CONFIG");
    assert_cmd::Command::from_std(cmd).args(["index","--full","--json"])
        .timeout(Duration::from_secs(120)).assert().success();
    let rows=canonical(&data);assert_eq!(rows.len(),2);
    for needle in ["shelleyalphaneedle","shelleybetaneedle"] {
        let result=search(home.path(),&data,&db,needle,None);
        assert_eq!(result["hits"].as_array().unwrap().len(),1,
            "explicit source must not scan the neighboring valid Shelley database");
        assert_eq!(result["hits"][0]["source_path"],db.to_string_lossy().as_ref());
    }
    assert_eq!(bundle(&db),before);assert_eq!(bundle(&neighbor),neighbor_before);
    assert_eq!(fs::read(&config_path).unwrap(),config_before);
    assert_eq!(storage_summary(&data).manifest_count,0);
    let mut doctor=command(home.path(),&data,&db);
    doctor.env_remove("CASS_SHELLEY_DB").env_remove("CASS_IGNORE_SOURCES_CONFIG");
    let output=assert_cmd::Command::from_std(doctor)
        .args(["doctor","archive-scan","--json","--verbose"])
        .timeout(Duration::from_secs(60)).assert().success().get_output().stdout.clone();
    let report:Value=serde_json::from_slice(&output).unwrap();
    let receipts=report["raw_mirror_backfill"]["receipts"].as_array().unwrap();
    assert_eq!(receipts.len(),2,"doctor must account for both canonical sessions");
    assert!(receipts.iter().all(|receipt|receipt["provider"]=="shelley"
        && receipt["action"]=="disabled_sensitive_container"
        && receipt["raw_source_captured"]==false));
    assert_eq!(report["raw_mirror_backfill"]["eligible_live_source_count"],0);
    assert_eq!(bundle(&db),before);assert_eq!(bundle(&neighbor),neighbor_before);
    assert_eq!(storage_summary(&data).manifest_count,0);
}
