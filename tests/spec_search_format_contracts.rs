...
fn robot_search_refresh_is_deferred_to_explicit_index_command() -> TestResult {
    ...
    let skipped = payload["budget"]["skipped_sections"]
        ...
        skipped.len() == 1 && skipped[0] == "refresh",
...
#[cfg(unix)]
#[test]
fn blocking_sessions_file_search_dry_run_recommends_exact_replayable_scope() -> TestResult {
    let tmp = TempDir::new()?;
    let data_dir = copy_search_demo_fixture(tmp.path())?;
    let sessions_fifo = tmp.path().join("dry-run-sessions.fifo");
    ensure(
    ...
    let payload: serde_json::Value = serde_json::from_slice(&output.stdout)?;
        ...
        payload["budget"]["timed_out"] == true,
        format!("dry-run FIFO timeout omitted timed_out=true: {payload}"),
    )?;
    ensure(
        payload["budget"]["skipped_sections"]
            .as_array()
            .is_some_and(|sections| sections.iter().any(|value| value == "sessions_from")),
        format!("dry-run FIFO timeout did not identify its blocked input: {payload}"),
    )?;
    let retry = payload["budget"]["recommended_next_probe"]
        .as_str()
        .ok_or_else(|| test_error("dry-run FIFO timeout omitted replayable retry"))?;
    ensure(
        retry.contains("cass search hello")
            && retry.contains("--robot")
            && retry.contains("--robot-meta")
            && retry.contains("--dry-run")
            && retry.contains("--timeout 1000")
            && retry.contains("--mode lexical")
            && retry.contains("--agent codex")
            && retry.contains("--workspace /tmp/cass-dry-run-workspace")
            && retry.contains("--source local")
            && retry.contains("--since 1700000000")
            && retry.contains("--until 1700000100")
            && retry.contains("--request-id dry-run-budget-retry")
            && retry.contains("--sessions-from")
            && retry.contains(&sessions_fifo.display().to_string())
            && retry.contains("--data-dir"),
        format!("dry-run retry lost its exact file scope: {retry}"),
    )?;
    Ok(())
}

#[test]
fn empty_sessions_file_search_is_a_completed_zero_hit_scope() -> TestResult {
    let tmp = TempDir::new()?;
    let data_dir = copy_search_demo_fixture(tmp.path())?;
    let sessions_file = tmp.path().join("empty-sessions.txt");
    ...
    let output = Command::cargo_bin("cass")?
        ...
        output.status.success(),
        format!(
            "empty-session-scope search failed: status={:?}; stderr={}",
            output.status,
            String::from_utf8_lossy(&output.stderr)
        ),
    )?;
    ...
    let payload: serde_json::Value = serde_json::from_slice(&output.stdout)?;
        ...
        payload["budget"]["timed_out"] == false,
        format!("empty session file was misreported as a timeout: {payload}"),
    )?;
    ensure(
        payload["budget"]["skipped_sections"]
            .as_array()
            .is_some_and(Vec::is_empty),
        format!("completed empty session scope shed work: {payload}"),
    )?;
    ensure(
        payload["hits"].as_array().is_some_and(Vec::is_empty),
        format!("empty session file broadened into archive hits: {payload}"),
    )?;
    Ok(())
}
