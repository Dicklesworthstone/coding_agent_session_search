...
/// Re-encode the `.14.2` planner's state/sidecar decision so the real-binary
/// gate proves doctor is projecting the planner's exact branch, not a
/// hand-written lookalike. The WAL-vs-SHM split is intentionally filesystem
/// derived because both share the same `wal_sidecar_suspect` diagnosis.
fn expected_storage_recovery_path(storage_state: &str, data_dir: &Path) -> &'static str {
    match storage_state {
        "ok" => "no_action_needed",
        "derived_only_drift" => "rebuild_derived_index",
        "busy_or_locked" => "wait_for_writer",
        "wal_sidecar_suspect" if data_dir.join("agent_search.db-wal").exists() => {
            "checkpoint_recover_wal"
        }
        "wal_sidecar_suspect" if data_dir.join("agent_search.db-shm").exists() => {
            "recreate_sidecar"
        }
        "wal_sidecar_suspect" => "checkpoint_recover_wal",
        "schema_drift" | "legacy_interop_failed" => "migrate_legacy_metadata",
        "fts_metadata_failed" => "targeted_fts_repair",
        "openread_failed" | "integrity_failed" => "archive_reconstruction",
        "unsafe_sql_shape" | "unknown_deferred" | "unchecked" => "inspect_refused",
        _ => "INVALID",
    }
}

/// Validate the typed `.14.2` salvage plan projected by doctor. This checks
/// diagnosis parity, exact path selection, sidecar evidence, and the
/// backup-first/non-destructive envelope. It deliberately validates structure
/// and safety rather than scraping prose.
fn storage_recovery_plan_problems(
    payload: &Value,
    data_dir: &Path,
    fixture_id: &str,
    storage_state: &str,
    ...
    let Some(plan) = payload
        .get("storage_recovery_plan")
        .and_then(Value::as_object)
    else {
        problems.push(format!(
            "[{fixture_id}] doctor JSON is missing the typed storage_recovery_plan"
        ));
        return problems;
    };

    let string_field = |field: &str| plan.get(field).and_then(Value::as_str).unwrap_or("");
    if string_field("plan_kind")
        .cmp("storage_salvage_plan")
        .is_ne()
    {
        problems.push(format!(
            "[{fixture_id}] storage recovery plan_kind {:?} != storage_salvage_plan",
            string_field("plan_kind")
        ));
    }
    let mut check_diagnosis_field = |field: &str, observed: &str, expected: &str| {
        if observed.cmp(expected).is_ne() {
            problems.push(format!(
                "[{fixture_id}] storage recovery {field} {observed:?} != diagnosis {expected:?}"
            ));
        }
    };
    check_diagnosis_field(
        "storage_state",
        string_field("storage_state"),
        storage_state,
    );
    check_diagnosis_field(
        "source_of_truth_risk",
        string_field("source_of_truth_risk"),
        risk,
    );
    check_diagnosis_field(
        "archive_readability",
        string_field("archive_readability"),
        archive,
    );

    let expected_path = expected_storage_recovery_path(storage_state, data_dir);
    let repair_path = string_field("repair_path");
    if repair_path.cmp(expected_path).is_ne() {
        problems.push(format!(
            "[{fixture_id}] storage recovery repair_path {repair_path:?} != planner branch {expected_path:?}"
        ));
    }
    let expected_db_path = db_path(data_dir).display().to_string();
    if string_field("source_db_path")
        .cmp(expected_db_path.as_str())
        .is_ne()
    {
        problems.push(format!(
            "[{fixture_id}] storage recovery source_db_path {:?} != {expected_db_path:?}",
            string_field("source_db_path")
        ));
    }

    let wal_present = data_dir.join("agent_search.db-wal").exists();
    let shm_present = data_dir.join("agent_search.db-shm").exists();
    let sidecars = plan.get("sidecars").and_then(Value::as_object);
    let mut check_sidecar_field = |field: &str, expected: bool| {
        let observed = sidecars
            .and_then(|value| value.get(field))
            .and_then(Value::as_bool);
        if observed.cmp(&Some(expected)).is_ne() {
            problems.push(format!(
                "[{fixture_id}] storage recovery sidecars.{field} {observed:?} != {expected}"
            ));
        }
    };
    check_sidecar_field("wal_present", wal_present);
    check_sidecar_field("shm_present", shm_present);

    if !matches!(
        plan.get("never_deletes_source_evidence")
            .and_then(Value::as_bool),
        Some(true)
    ) {
        problems.push(format!(
            "[{fixture_id}] storage recovery did not assert never_deletes_source_evidence=true"
        ));
    }
    if let Some(checks) = plan.get("checks_attempted").and_then(Value::as_array)
    ...
    {
        problems.push(format!(
            "[{fixture_id}] storage recovery copied a non-read-only diagnostic check"
        ));
    }

    let mutation_scope = string_field("mutation_scope");
    let backup_required = plan.get("backup_required").and_then(Value::as_bool);
    let requires_confirmation = plan.get("requires_confirmation").and_then(Value::as_bool);
    let low_risk_derived = plan.get("low_risk_derived_repair").and_then(Value::as_bool);
    if mutation_scope.cmp("archive").is_eq() {
        if !matches!(backup_required, Some(true))
            || !matches!(requires_confirmation, Some(true))
            || !matches!(low_risk_derived, Some(false))
        {
            problems.push(format!(
                "[{fixture_id}] archive recovery path is not backup+confirmation gated \
                 (backup_required={backup_required:?}, requires_confirmation={requires_confirmation:?}, \
                 low_risk_derived_repair={low_risk_derived:?})"
            ));
        }
        if plan
            .get("proposed_backup_path")
            .and_then(Value::as_str)
            .is_none()
        {
            problems.push(format!(
                "[{fixture_id}] archive recovery path has no proposed_backup_path"
            ));
        }
        let blocked_apply = plan
            ...
            .is_some_and(|commands| {
                commands.iter().any(|command| {
                    command
                        ...
                        .and_then(Value::as_str)
                        .is_some_and(|command| {
                            command.contains("--yes") && command.contains("--plan-fingerprint")
                        })
                })
            });
        if !blocked_apply {
            problems.push(format!(
                "[{fixture_id}] archive recovery path did not keep fingerprinted apply blocked"
            ));
        }
        if let Some(mutations) = plan.get("expected_mutations").and_then(Value::as_array)
            && mutations.iter().any(|mutation| {
                !matches!(
                    mutation.get("backed_up_first").and_then(Value::as_bool),
                    Some(true)
                )
            })
        {
            problems.push(format!(
                "[{fixture_id}] archive recovery has an expected mutation not backed up first"
            ));
        }
    } else if repair_path.cmp("rebuild_derived_index").is_eq() {
        if !matches!(backup_required, Some(false))
            || !matches!(requires_confirmation, Some(false))
            || !matches!(low_risk_derived, Some(true))
        {
            problems.push(format!(
                "[{fixture_id}] derived-only recovery was not classified as the sole low-risk \
                 repair (backup_required={backup_required:?}, requires_confirmation={requires_confirmation:?}, \
                 low_risk_derived_repair={low_risk_derived:?})"
            ));
        }
    } else if mutation_scope.cmp("none").is_eq() && !matches!(backup_required, Some(false)) {
        problems.push(format!(
            "[{fixture_id}] non-mutating recovery path unexpectedly requires a backup"
        ));
    }

    if let Some(command) = plan
        .pointer("/recommended_command/command")
        .and_then(Value::as_str)
        && (command.trim().cmp("cass").is_eq()
            || command.contains(" --yes")
            || command.contains(" --plan-fingerprint"))
    {
        problems.push(format!(
            "[{fixture_id}] recommended recovery command is bare or mutating: {command:?}"
        ));
    }
...
fn validate_storage_recovery_surface(
    payload: &Value,
    data_dir: &Path,
    fixture_id: &str,
    ...
    let execution_mode = payload
        .pointer("/doctor_command/execution_mode")
        .and_then(Value::as_str)
        .unwrap_or("");
    if execution_mode.cmp(expected_execution_mode).is_ne() {
        return Err(format!(
            "[{fixture_id}] doctor execution_mode {execution_mode:?} != {expected_execution_mode:?}"
        ));
    }
    let diagnosis = payload
        .get("storage_integrity")
        .and_then(Value::as_object)
        .ok_or_else(|| format!("[{fixture_id}] missing storage_integrity block"))?;
    let storage_state = diagnosis
        .get("storage_state")
        .and_then(Value::as_str)
        .unwrap_or("");
    let risk = diagnosis
        ...
        .and_then(Value::as_str)
        .unwrap_or("");
    let archive = diagnosis
        .get("archive_readability")
        .and_then(Value::as_str)
        .unwrap_or("");
    let problems =
        storage_recovery_plan_problems(payload, data_dir, fixture_id, storage_state, risk, archive);
    if problems.is_empty() {
        Ok(())
    } else {
        Err(problems.join("\n  - "))
...
fn probe_doctor_storage_state(
    ...
    // 5. The same live diagnosis drives a typed, backup-first `.14.2` salvage
    //    plan. This is the real-binary bridge between the report and planner.
    problems.extend(storage_recovery_plan_problems(
        &value, fixture_dd, fixture.id, &observed, &risk, &archive,
    ));
    // 6. The live storage_state matches the per-fixture contract — exact for the
    ...
    // 7. doctor --check is read-only: the canonical DB is byte-identical for a
...
fn doctor_check_projects_storage_state_per_fixture() -> Result<(), String> {
    ...
    // Strict real-binary dry-run proof: reuse one already-corrupted canonical
    // fixture, require the bounded command to finish (no timeout skip), assert
    // the same typed backup-first envelope, and prove the canonical DB bytes
    // remain unchanged. The six planner branches themselves are exhaustively
    // checked by the focused adapter unit test; this pins the CLI mode bridge.
    let dry_run_fixture = suite
        .iter()
        .find(|fixture| fixture.id.cmp("fm-storage-pragma-integrity-fail").is_eq())
        .ok_or_else(|| "missing strict dry-run storage fixture".to_string())?;
    let dry_run_dd = home
        .path()
        .join(format!("doctor-state-{}", dry_run_fixture.id));
    let dry_run_data_dir = dry_run_dd
        .to_str()
        .ok_or_else(|| "strict dry-run data dir is not UTF-8".to_string())?;
    let dry_run_hash_before = sha256_hex(
        &std::fs::read(db_path(&dry_run_dd))
            .map_err(|err| format!("read strict dry-run DB before probe: {err}"))?,
    );
    let dry_run_out = run_surface_caught(
        home.path(),
        &codex_home,
        "fm-storage-strict-repair-dry-run",
        &[
            "doctor",
            "repair",
            "--dry-run",
    ...
    .ok_or_else(|| "strict doctor repair --dry-run timed out".to_string())?;
    let dry_run_payload =
        parse_complete_json(dry_run_out, "strict doctor repair --dry-run", &[0, 5])?;
    validate_storage_recovery_surface(
        &dry_run_payload,
        &dry_run_dd,
        dry_run_fixture.id,
        "repair-dry-run",
    )?;
    check_db_preserved(&dry_run_dd, &dry_run_hash_before, true)?;
