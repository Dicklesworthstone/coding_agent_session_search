...
| H.1 verification | 8 of the 12 Pages/secret-scan beads verified against code and closed with landing commits (cc7pi, z9sg6, yjjsg, kjdbv, jfcgi, h0rss, c8gx1, 1hg2q); 4 left open with dated notes naming the unproven items (h3ibc, 4ydds, 7y2jt, 45jxv) | `br` |

**The golden leak, root-caused.** The two goldens that absorbed a fleet worker's real Pi Agent sessions were not fed by connector detection at all: `isolated_search_demo_data` copies a prebuilt fixture archive and never runs `cass index`. The writer was stale-on-read auto-refresh: it is suppressed for data dirs under the OS temp dir, but rch workers place `tempfile` directories beneath the checkout, so a golden `search`/`stats` spawned a detached `cass index --background` into the fixture copy, which ingested the worker's sessions (raw mirror initialized, 10 conversations) before the next command in the same data dir read them. `CASS_AUTO_REFRESH=0` in the harness removes the writer; the Pi env pins stay as belt-and-braces. The two leaked goldens were restored to HEAD, not committed; verify7 re-checks them on the fleet.

**In the tree, not yet wired (batch 2, verify7):** `src/pages/key_cli.rs` — `cass pages key list|add-password|add-recovery|revoke|rotate` over the complete `key_management` API (four unit tests on a real encrypted bundle, planted negatives for wrong password and last-slot revoke), plus `docs/RECOVERY.md` rewritten to the real surface, the README `pages key` row, the dead `Alt+W` swarm arm removed, and the CHANGELOG `[Unreleased]` entry.
