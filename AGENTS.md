...
|------------|-----------------|
| `frankensqlite` / `fsqlite-types` | crates.io `=0.3.14` (bumped from 0.3.13 in 5acc0dc6 on 2026-09-01 for the FTS5 stock-compat and checkpoint-watermark fixes. 0.3.15 was evaluated on 2026-09-02 and NOT adopted — bead gh382-fsqlite-pin: a cass release build on it still loops in the writable open on the owner's 10 GB archive with a 200 MB WAL (`reclaim_disowned_in_range` rescans the WAL per ledger page, GH #382 / bead g3zyo) and its open is ~5 s slower on a fresh archive; the loop is fixed upstream in frankensqlite `8d012706a` and cass moves straight to the release that carries it. asupersync-0.4.3 migration; GH#333/GH#334 fixes including cass#393; allocator/freelist/concurrent-writer correctness work; FTS5 hardening; Windows sidecar-less read-only close repair; checkpoint/recovery and CHECK-enforcement hardening; cass#434 autoindex-vanish corruption-writer fixes). `build.rs` enforces one exact crates.io source/version and rejects fsqlite-family registry patches. |
