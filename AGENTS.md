...
|------------|-----------------|
| `frankensqlite` / `fsqlite-types` | crates.io `=0.3.15` (2026-09-02, bead gh382-fsqlite-pin: on the owner's 10 GB archive with a 200 MB WAL, 0.3.14's writable open never returns while 0.3.15's open and `BEGIN IMMEDIATE` complete; 0.3.15 also carries the FTS5 `optimize` migration and contentless data-safety fixes. Still open in 0.3.15: `PRAGMA wal_checkpoint` on such an archive loops in `reclaim_disowned_in_range` — fixed upstream in frankensqlite `8d012706a`, ships in the next release, bead g3zyo. 0.3.14 was the 2026-09-01 bump (5acc0dc6) for the FTS5 stock-compat and checkpoint-watermark fixes; asupersync-0.4.3 migration; GH#333/GH#334 fixes including cass#393; allocator/freelist/concurrent-writer correctness work; FTS5 hardening; Windows sidecar-less read-only close repair; checkpoint/recovery and CHECK-enforcement hardening; cass#434 autoindex-vanish corruption-writer fixes). `build.rs` enforces one exact crates.io source/version and rejects fsqlite-family registry patches. |
