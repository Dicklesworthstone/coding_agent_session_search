...
- **Upgrade behavior**: archives created by older cass versions are reclassified once from `pi_agent` to `omp` using the same conservative canonical/XDG/remote-mirror path classifier as live ownership, then the derived lexical index and analytics are rebuilt so a transcript cannot remain attributed to both agents
