...
| `frankensearch` | crates.io `=0.4.0` (2026-08-24, gh#416: the first registry release of the current line, published bottom-up with every sub-crate at a fresh version; supersedes the git rev `22859f74` pin and everything it carried — CASS→Quill lexical flip with `cass-compat` → `lexical-tantivy` retained for the differential oracle and the schema-generation sentinel, pure-Rust `native`, architecture-safe HNSW, consumer-owned `TwoTierIndexPaths`, non-mutating lexical admission, cancellation-safe facade opening, generation-pinned Quill hydration. Registry `0.3.2` is a stale same-version twin of an older tree (no quill/cass-compat/native) — the exact pin exists so resolution can never reach it. frankentorch now resolves from crates.io as `frankentorch-*`; the HNSW fork is registry `frankenhnsw 0.3.5`; tantivy is registry `=0.26.1` (RUSTSEC-2026-0253 on its lru is unreachable: the advisory needs a panicking `Drop` on a cache key under `catch_unwind`, and tantivy's cache keys are trivially droppable) — cass #308, #333, bd-8nqz.5, bd-07os, bd-r65a.1) |
