        ...
        commands: &[
            ...
            ScenarioCommand {
                ...
                env: &[
                    ...
                    ScenarioEnv {
                        key: "CASS_TEST_FLEET_PROBE_SLOW_MS",
                        value: "2000",
                    },
                ],
                accepted_exit_codes: &[1],
...
mod tests {
    ...
    fn required_report_scenarios_are_present() {
        ...
        assert_eq!(
            ci_scenarios()
                ...
                .sum::<usize>(),
            11,
            "the deterministic matrix must execute all eleven report-derived commands"
        );
    }

    #[test]
    fn deterministic_scenario_command_keys_are_unique() {
    ...
    fn every_scenario_is_fully_specified() {
        for s in e2e_scenarios() {
            ...
            assert!(!s.commands.is_empty(), "{} commands", s.id);
            for command in s.commands {
                assert!(!command.id.is_empty(), "{} command id", s.id);
                assert!(!command.args.is_empty(), "{} command args", s.id);
                assert!(
                    !command.json_assertions.is_empty(),
                    "{} command {} assertions",
                    s.id,
                    command.id
                );
    ...
    #[test]
    fn every_assertion_is_machine_evaluable() {
        ...
        for assertion in [
            ...
            assert!(assertion.evaluate(&fixture), "{}", assertion.label());
        }
    }
}
