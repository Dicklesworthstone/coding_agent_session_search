...
impl GrokBotConnector {
    ...
    fn local_sources(&self, ctx: &ScanContext) -> Result<Vec<DiscoveredSourceFile>> {
        ...
        if sources.iter().any(|source| source.origin.is_remote()) {
            bail!("Grok Bot remote replicas are disabled because they contain non-chat secrets and approval payloads; index the original local store with CASS_GROK_BOT_DATA_ROOT");
        }
        Ok(sources)
    }
}
...
impl Connector for GrokBotConnector {
    fn detect(&self) -> DetectionResult { self.inner.detect() }
    ...
    fn supports_streaming_scan(&self) -> bool { true }

    fn supports_source_boundaries(&self) -> bool { true }

    fn scan_with_callback(&self, ctx: &ScanContext, on_conversation: &mut dyn FnMut(NormalizedConversation) -> Result<()>) -> Result<()> {
        self.scan_with_source_boundaries(ctx, &mut franken_agent_detection::connectors::SourceScanHooks::default(), on_conversation)
    }

    fn scan_with_source_boundaries(&self, ctx: &ScanContext, hooks: &mut franken_agent_detection::connectors::SourceScanHooks<'_>, on_conversation: &mut dyn FnMut(NormalizedConversation) -> Result<()>) -> Result<()> {
        ...
        self.inner.scan_with_source_boundaries(&local, hooks, on_conversation)
    }
}
...
mod tests {
    ...
    fn gh447_grok_bot_local_chat_and_remote_refusal_preserve_grok_cli_identity() {
        ...
        let bits: Vec<_> = key.bytes().flat_map(|byte| (0..8).rev().map(move |bit| (byte >> bit) & 1)).collect();
        let encoded: String = bits.chunks(5).map(|chunk| {
        ...
        let local = ScanContext::with_roots(temp.path().join("data"), vec![ScanRoot::local(path.clone())], None);
        ...
        assert!(!serde_json::to_string(&conversations).unwrap().contains("excluded secret"));
        let remote = ScanContext::with_roots(temp.path().join("data"), vec![ScanRoot::remote(path.clone(), Origin::remote("laptop"), None)], None);
        assert!(connector.discover_source_files(&remote).is_err());
        assert!(connector.scan(&remote).is_err());
        let mut callbacks = 0;
        assert!(connector.scan_with_callback(&remote, &mut |_| { callbacks += 1; Ok(()) }).is_err());
        assert!(connector.scan_with_source_boundaries(&remote, &mut SourceScanHooks::default(), &mut |_| { callbacks += 1; Ok(()) }).is_err());
        assert_eq!(callbacks, 0);
        assert_eq!(std::fs::read(&path).unwrap(), bytes);
        let unrelated = ScanContext::with_roots(temp.path().join("data"), vec![ScanRoot::remote(temp.path().join("codex"), Origin::remote("laptop"), None)], None);
        assert!(connector.scan(&unrelated).unwrap().is_empty());
        assert!(GrokConnector::new().scan(&local).unwrap().is_empty(), "Grok CLI must not claim Grok Bot replicas");
        let factory = crate::connectors::get_connector_factories().into_iter().find(|(slug, _)| *slug == "grok_bot").unwrap().1;
        assert!(factory().scan(&remote).is_err(), "runtime factory must apply the CASS boundary");
    }
}
