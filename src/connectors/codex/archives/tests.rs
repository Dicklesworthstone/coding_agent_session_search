...
fn archive_moves_keep_full_home_ids_and_old_files_remain_eligible() -> Result<()> {
    ...
    let after = archived.iter().find(|item| item.source_path == moved).unwrap();
    ...
    assert_eq!(serde_json::to_value(&restored)?, serde_json::to_value(&before)?);
    Ok(())
}
...
fn archive_only_home_has_the_same_identity_as_a_home_with_active_sessions() -> Result<()> {
    ...
    let expected = both.iter().find(|item| item.source_path == paths[1]).unwrap();
...
fn remote_archive_provenance_and_scoped_ids_are_preserved() -> Result<()> {
    ...
    assert_eq!(connector.scan(&narrow)?[0].external_id, native[0].external_id);
    Ok(())
}
...
fn archive_exclusions_and_default_codex_home_are_isolated() -> Result<()> {
    ...
    let scenarios = [
        ...
        (home.join("sessions").display().to_string(), vec![paths[1].clone()]),
        (home.join(ARCHIVE).display().to_string(), vec![paths[0].clone()]),
