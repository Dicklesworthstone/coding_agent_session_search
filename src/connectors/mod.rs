...
pub mod scan_activity {
    ...
    type TickSink = Arc<dyn Fn() + Send + Sync>;
    type SinkRegistry = Vec<(u64, TickSink)>;
    ...
    fn lock_registry(registry: &Mutex<SinkRegistry>) -> MutexGuard<'_, SinkRegistry> {
        registry
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
    }

    fn lock_sinks() -> MutexGuard<'static, SinkRegistry> {
        lock_registry(&SINKS)
    }

    fn snapshot_registry(registry: &Mutex<SinkRegistry>) -> Vec<TickSink> {
    ...
    pub fn register(tick: Box<dyn Fn() + Send + Sync>) -> ScanActivityGuard {
        ...
        lock_sinks().push((id, TickSink::from(tick)));
        ScanActivityGuard { id }
    }

    impl Drop for ScanActivityGuard {
        fn drop(&mut self) {
            lock_sinks().retain(|(id, _)| *id != self.id);
        }
    }
    ...
    pub fn tick() {
        // Never invoke external code while holding the registry mutex. Apart
        // from extending the critical section across every callback, doing so
        // deadlocks if a callback registers/drops another sink. A callback
        // panic would also poison the global registry and silently disable all
        // future index-run heartbeats. Arc snapshots make callback lifetime
        // explicit while the lock is released.
        for sink in snapshot_registry(&SINKS) {
            sink();
        }
    }
    ...
    mod tests {
        ...
        #[test]
        fn callbacks_can_reenter_the_registry() -> Result<(), String> {
            // ubs:ignore — isolated test registry intentionally mirrors the production mutex.
            let registry: Arc<Mutex<SinkRegistry>> = Arc::new(Mutex::new(Vec::new()));
            let reentered = Arc::new(AtomicUsize::new(0));
            let callback_count = Arc::clone(&reentered);
            let callback_registry = Arc::clone(&registry);
            let callback: TickSink = Arc::new(move || {
                lock_registry(&callback_registry).push((99, Arc::new(|| {})));
                callback_count.fetch_add(1, Ordering::Relaxed);
            });
            lock_registry(&registry).push((1, callback));

            for sink in snapshot_registry(&registry) {
                sink();
            }
            let observed = (
            ...
            (observed == (1, 2))
                .then_some(())
                .ok_or_else(|| format!("unexpected callback/registry counts: {observed:?}"))
        }

        #[test]
        fn panicking_callback_does_not_poison_the_registry() -> Result<(), String> {
            let registry = Mutex::new(vec![(
                1,
                Arc::new(|| -> () { panic!("injected heartbeat panic") }) as TickSink,
            )]);
            let callbacks = snapshot_registry(&registry);
            let panic = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                for sink in callbacks {
                    sink();
                }
            }));
            let registry_remained_healthy = registry.lock().is_ok();

            let count = Arc::new(AtomicUsize::new(0));
            let sink_count = Arc::clone(&count);
            lock_registry(&registry).retain(|(id, _)| *id != 1);
            lock_registry(&registry).push((
                2,
                Arc::new(move || {
                    sink_count.fetch_add(1, Ordering::Relaxed);
                }),
            ...
            let observed = (
                panic.is_err(),
                registry_remained_healthy,
                count.load(Ordering::Relaxed),
            );
            (observed == (true, true, 1)).then_some(()).ok_or_else(|| {
                format!("callback panic did not preserve registry health: {observed:?}")
            })
        }
    }
}
