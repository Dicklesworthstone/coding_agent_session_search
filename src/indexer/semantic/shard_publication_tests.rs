...
fn rows(indexer: &SemanticIndexer, text: &str) -> Vec<EmbeddedMessage> {
    (1..=3).map(|message_id| EmbeddedMessage {
        message_id,
        created_at_ms: 100,
        agent_id: 1,
        workspace_id: 2,
...
fn snapshot(root: &Path) -> BTreeMap<PathBuf, Vec<u8>> {
    ...
    for entry in fs::read_dir(root).unwrap() {
        ...
        else { files.insert(path.clone(), fs::read(path).unwrap()); }
    }
    files
}
...
fn assert_preserved(files: &BTreeMap<PathBuf, Vec<u8>>) {
    for (path, bytes) in files {
        assert_eq!(&fs::read(path).unwrap(), bytes, "published file changed: {}", path.display());
    }
}
...
fn repeated_source_fingerprint_publishes_new_paths_and_preserves_old_native_readers() {
    ...
    let indexer = indexer();
    let old = indexer.build_and_save_index_shards(rows(&indexer, "old compiler storage"), temp.path(), plan(TierKind::Fast, true)).unwrap();
    ...
    let new = indexer.build_and_save_index_shards(rows(&indexer, "new astronomy planets"), temp.path(), plan(TierKind::Fast, true)).unwrap();
    assert!(old.complete && new.complete);
    assert!(new.index_paths.iter().all(|path| !old.index_paths.contains(path)), "a rebuild must not reuse installed vector paths");
    assert!(new.ann_index_paths.iter().all(|path| !old.ann_index_paths.contains(path)), "a rebuild must not reuse installed ANN paths");
    ...
    for (ordinal, record) in manifest.shards.iter().enumerate() {
        ...
        assert!(FsHnswIndex::try_load_native(&graph, &source).unwrap().is_some());
    }
}
...
fn failure_after_one_new_shard_keeps_every_installed_file_and_manifest_unchanged() {
    let temp = tempfile::tempdir().unwrap();
    let indexer = indexer();
    let old = indexer.build_and_save_index_shards(rows(&indexer, "old stable archive"), temp.path(), plan(TierKind::Fast, true)).unwrap();
    let old_files = snapshot(old.index_paths[0].parent().unwrap());
    ...
    let result = indexer.build_and_save_index_shards(replacement, temp.path(), plan(TierKind::Fast, true));
    assert!(result.is_err());
    assert_eq!(fs::read(SemanticShardManifest::path(temp.path())).unwrap(), manifest_before);
    assert_preserved(&old_files);
    let manifest = SemanticShardManifest::load(temp.path()).unwrap().unwrap();
    assert!(manifest.summary(TierKind::Fast, indexer.embedder_id(), &plan(TierKind::Fast, true).db_fingerprint).complete);
}

#[test]
fn malformed_or_future_manifest_is_not_discarded_before_a_new_build() {
    for bytes in [b"not-json".as_slice(), br#"{"manifest_version":4294967295,"shards":[],"updated_at_ms":0}"#.as_slice()] {
        let temp = tempfile::tempdir().unwrap();
        let path = SemanticShardManifest::path(temp.path());
        fs::create_dir_all(path.parent().unwrap()).unwrap();
        fs::write(&path, bytes).unwrap();
        let before = snapshot(temp.path());
        let inputs = std::iter::from_fn(|| -> Option<EmbeddedMessage> { panic!("invalid manifest must reject before input consumption") });
        let result = indexer().build_and_save_index_shards(inputs, temp.path(), plan(TierKind::Fast, false));
        assert!(result.is_err());
...
fn corrupt_manifest_observed_at_publish_is_preserved_not_overwritten() {
    ...
    assert!(indexer.build_and_save_index_shards(replacement, temp.path(), plan(TierKind::Fast, false)).is_err());
    assert!(changed);
    assert_eq!(fs::read(&path).unwrap(), b"externally changed invalid manifest");
}
...
fn publishing_one_tier_keeps_the_other_tiers_selected_files() {
    let temp = tempfile::tempdir().unwrap();
    let indexer = indexer();
    let quality = indexer.build_and_save_index_shards(rows(&indexer, "quality control"), temp.path(), plan(TierKind::Quality, false)).unwrap();
    let before = snapshot(quality.index_paths[0].parent().unwrap());
    let fast = indexer.build_and_save_index_shards(rows(&indexer, "fast control"), temp.path(), plan(TierKind::Fast, false)).unwrap();
    assert!(fast.complete && quality.complete);
    let manifest = SemanticShardManifest::load(temp.path()).unwrap().unwrap();
    assert_eq!(manifest.shards.iter().filter(|s| s.tier == TierKind::Quality).map(|s| temp.path().join(&s.index_path)).collect::<Vec<_>>(), quality.index_paths);
...
fn huge_shard_limit_does_not_allocate_before_receiving_a_small_input() {
    ...
    let outcome = indexer.build_and_save_index_shards(rows(&indexer, "bounded inputs"), temp.path(), plan).unwrap();
