...
impl DestinationLock {
    pub fn acquire(destination: &Path) -> Result<Self> {
        ...
        let name = destination.file_name().ok_or_else(|| anyhow!("destination requires a file name"))?;
        let mut lock_name = std::ffi::OsString::from(".");
        lock_name.push(name);
        lock_name.push(".logical-archive.lock");
        let path = parent.join(lock_name);
        if let Ok(metadata) = fs::symlink_metadata(&path) {
            ensure!(metadata.is_file() && !metadata.file_type().is_symlink(), "destination lock is not a regular file");
        }
        ...
        let file = options.open(&path).context("cannot open logical archive destination lock")?;
        ensure!(file.metadata()?.is_file(), "destination lock is not a regular file");
        #[cfg(windows)]
        {
            use std::os::windows::fs::MetadataExt;
            ensure!(file.metadata()?.file_attributes() & 0x400 == 0, "destination lock is a reparse point");
        }
        ...
        loop {
            match FileExt::try_lock_exclusive(&file) {
                ...
                Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => {
                    ensure!(Instant::now() < deadline, "logical archive destination remained locked for five seconds");
...
pub fn parent(path: &Path) -> Result<&Path> {
    path.parent().map(|parent| if parent.as_os_str().is_empty() { Path::new(".") } else { parent })
        .ok_or_else(|| anyhow!("destination requires a parent directory"))
}

pub fn path_text(path: &Path) -> Result<&str> {
    path.to_str().ok_or_else(|| anyhow!("FrankenSQLite database paths must be valid UTF-8"))
}
...
pub fn open_source(path: &Path) -> Result<Connection> {
    ...
    ensure!(metadata.is_file() && !metadata.file_type().is_symlink(), "source archive must be a regular, non-symlink file");
...
pub fn schema_version(connection: &Connection) -> Result<String> {
    connection.query_row("SELECT value FROM meta WHERE key = 'schema_version'")?
        .get_typed::<String>(0)
...
pub fn tables(connection: &Connection) -> Result<Vec<Table>> {
    ...
    ensure!(rows.len() <= 1024, "source has too many schema objects for logical export");
    ...
    for name in physical {
        ...
        ensure!(output.len() < codec::MAX_TABLES, "source exceeds 256 logical tables");
        let columns = connection.query(&format!("PRAGMA table_info({})", quoted(&name)?))?;
        ensure!(columns.len() <= codec::MAX_COLUMNS, "source table exceeds 256 columns");
        ...
        for (offset, column) in columns.into_iter().enumerate() {
            ...
            if ordinal > 0 { primary_key.push((ordinal, offset)); }
        }
        primary_key.sort_unstable();
        table.primary_key = primary_key.into_iter().map(|(_, offset)| offset).collect();
        table.validate().with_context(|| format!("unsupported logical table {}", table.name))?;
        output.push(table);
    }
    ensure!(!output.is_empty(), "source contains no canonical logical tables");
    Ok(output)
}
...
pub fn cells(values: &[SqliteValue]) -> Result<Vec<Cell>> {
    ...
    for value in values {
        let size = match value {
            ...
            SqliteValue::Blob(blob) => blob.len().checked_add(2).and_then(|n| n.checked_div(3)).and_then(|n| n.checked_mul(4)).ok_or_else(|| anyhow!("oversized logical BLOB"))?,
            _ => 32,
        };
        bytes = bytes.checked_add(size).ok_or_else(|| anyhow!("oversized logical row"))?;
        ensure!(bytes < codec::MAX_RECORD_BYTES, "logical row exceeds 8 MiB");
    }
    values.iter().map(|value| Ok(match value {
...
pub fn snapshot(
    ...
    let header = Header {
        format: codec::FORMAT.to_owned(), schema_version: codec::VERSION,
        archive_id, exported_at_ms: chrono::Utc::now().timestamp_millis(),
        storage_schema_version: schema_version(connection)?,
        record_types: ["table", "row", "completion"].into_iter().map(str::to_owned).collect(),
        contains_private_data: true, omissions: vec!["derived_search_assets".to_owned()],
    };
    let mut validator = Validator::new(header.clone())?;
    output.write_all(&codec::encode(&Record::Header { header: header.clone() })?)?;
    for table in tables {
        output.write_all(&validator.push(&Record::Table { table: table.clone() })?)?;
        let columns = table.columns.iter().map(|name| quoted(name)).collect::<Result<Vec<_>>>()?.join(", ");
        let order = table.primary_key.iter().map(|&offset| {
            Ok(format!("{} COLLATE BINARY ASC", quoted(&table.columns[offset])?))
        }).collect::<Result<Vec<_>>>()?.join(", ");
        let sql = format!("SELECT {columns} FROM {} ORDER BY {order}", quoted(&table.name)?);
        let mut failure = None;
        let streamed = connection.query_with_params_for_each(&sql, &[], |row| {
            let result = (|| -> Result<()> {
                let record = Record::Row { values: cells(row.values())? };
                output.write_all(&validator.push(&record)?)?;
                Ok(())
            })();
            if let Err(error) = result {
                failure = Some(error);
                return Err(FrankenError::Internal("logical archive stream aborted".to_owned()));
            }
            Ok(())
        });
        if let Some(error) = failure { return Err(error).with_context(|| format!("logical table {}", table.name)); }
        streamed.map_err(|_| anyhow!("cannot stream logical table {}; source was not repaired", table.name))?;
    }
...
pub fn export_file(source: &Path, destination: &Path, archive_id: String) -> Result<(Header, Completion)> {
    let _lock = DestinationLock::acquire(destination)?;
    ensure!(fs::symlink_metadata(destination).is_err_and(|error| error.kind() == std::io::ErrorKind::NotFound), "export destination already exists or cannot be inspected; it was not replaced");
    ...
    ensure!(actual == result, "staged export failed read-back verification");
    temporary.persist_noclobber(destination).map_err(|_| anyhow!("cannot publish logical archive without replacing an existing file"))?;
...
pub fn sync_parent(destination: &Path) -> Result<()> {
    #[cfg(unix)]
    File::open(parent(destination)?)?.sync_all().context("archive published, but parent-directory durability could not be confirmed")?;
    #[cfg(not(unix))]
    let _ = destination;
    Ok(())
}
